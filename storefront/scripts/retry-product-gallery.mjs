// Retry missing product gallery images with Imagen 4 Ultra (separate quota).
// Scans for missing /img/<id>-<n>.jpg files and generates them.
...
console.log(`Probing ${ALL_KEYS.length} keys (${MODEL})...`);
const KEYS = [];
...
async function genImagen(prompt, aspectRatio) {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:predict?key=${pickKey()}`, {
...
// Build queue of MISSING files only
const queue = [];
for (const p of PRODUCTS) {
  for (const v of VARIANTS) {
    ...
    catch { queue.push({ p, v, slug, fp }); }
  }
}

console.log(`Missing ${queue.length} images. Generating with Imagen Ultra...\n`);
