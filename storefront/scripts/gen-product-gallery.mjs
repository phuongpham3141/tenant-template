// Generate gallery + description images for 24 home page products.
// Each product needs: 5 thumb images (id-1 to id-5) + 3 desc images (id-desc1 to id-desc3) = 8 per product.
// 24 × 8 = 192 images. Use Imagen Fast (cheap), no strict vision verify (just trust prompts).
...
// 24 home page products with detailed EN descriptions
const PRODUCTS = [
...
const VARIANTS = [
  { suffix: "1", w: 600, h: 600, ar: "1:1",  modifier: "main hero shot, centered on white background, professional product photography" },
  { suffix: "2", w: 600, h: 600, ar: "1:1",  modifier: "3/4 side angle view on white background, e-commerce product photo" },
  { suffix: "3", w: 600, h: 600, ar: "1:1",  modifier: "close-up detail of texture or material, isolated on white" },
  { suffix: "4", w: 600, h: 600, ar: "1:1",  modifier: "in a stylish installed setting or room context, lifestyle photo" },
  { suffix: "5", w: 600, h: 600, ar: "1:1",  modifier: "top-down or back view variant on white background" },
  { suffix: "desc1", w: 1200, h: 500, ar: "16:9", modifier: "wide cinematic hero image in elegant installed setting or luxury showroom context" },
  { suffix: "desc2", w: 600, h: 400, ar: "3:2", modifier: "close-up detail showing premium material, finish, or feature" },
  { suffix: "desc3", w: 600, h: 400, ar: "3:2", modifier: "in modern installed environment showcasing scale and use case" },
];
...
async function genImagen(prompt, aspectRatio) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/imagen-4.0-generate-001:predict?key=${pickKey()}`, {
...
for (const p of PRODUCTS) {
  ...
  for (const v of VARIANTS) {
    ...
    const prompt = `${v.modifier}: ${p.en}. Sharp focus, no text, no watermarks, no people.`;
    let success = false;
    for (let tryIdx = 0; tryIdx < 3 && !success; tryIdx++) {
      try {
        const raw = await genImagen(prompt, v.ar);
