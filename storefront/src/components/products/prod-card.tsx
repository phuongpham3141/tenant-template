  ...
  return (
    <div className="bg-paper border border-line rounded grid grid-cols-[220px_1fr_220px] gap-4 p-3.5 hover:border-brand hover:shadow-[0_4px_12px_rgba(0,60,143,0.08)] transition max-[900px]:grid-cols-[180px_1fr] max-[900px]:gap-3 max-md:grid-cols-1">
      ...
      <Link href={href} className="relative cursor-pointer block">
        ...
        <span className="absolute top-1.5 right-1.5 w-7 h-7 bg-white/90 rounded-full flex items-center justify-center text-[14px] text-mute hover:text-accent cursor-pointer" aria-label="Favorite">
          ♡
        </span>
        ...
        <span className="absolute bottom-1.5 right-1.5 w-6 h-6 bg-white/90 rounded-sm flex items-center justify-center text-[12px] text-mute cursor-pointer" aria-label="View larger image">
          ⌕
        </span>
      </Link>
      ...
      <div className="flex flex-col min-w-0 max-md:order-3">
        ...
        <div className="text-[12px] text-mute mb-2">
          <b className="text-ink">{p.moq}</b> (MOQ)
        </div>
        <Link href={href} className="cursor-pointer">
